...
// Add Content By User ID
const addContentById = (content_type) => async (req, res) => {
  const { content, name } = req.body;
  if (!content || !name ) {
    return res.status(400).json({ error: 'Content and Name are required' });
  }
  ...
  try {
    const testimonial = await Content.create({ content, name, content_type });
    res.status(201).json({ message: 'Content saved successfully', contentId: testimonial.id });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Database error' });
  }
